# My Experiments With Competitive Programming

## Why
I'm about to start the fifth semester of my graduation, yet I haven't focused on my data structures, programming fundamentals, algorithms, competitive programming, blah blah :cry:. So, I'm creating this repo/blog/website to get regular at cp(thus hopefully improving my solving skills). I will be experimenting with this domain for the next 10-12 months. So let's see how this experiment goes :crossed_fingers:

## Progress

|  |  Codeforces | Leetcode | Codechef | Project Euler 
|:--|:--|:--|:--|:--
| Nov | 874 | 1427 | 1578 | 13 
| Dec |  |  |  | 
| Jan |  |  |  | 
| Feb |  |  |  | 
| Mar |  |  |  | 
| Apr |  |  |  | 
| May |  |  |  | 
| Jun |  |  |  | 
| Jul |  |  |  | 
| Aug |  |  |  | 
| Sept|  |  |  | 
| Oct |  |  |  | 

## Some important checkpoints

#### By Jan, 2021

* [] Complete all the basic data structures
* [] Fundamental of C++
* [] STL
* [] Striver 180
* [] Pupil at codeforces
* [] 3* on Leetcode
* [] 3* star at codechef
* [] 50+ on project euler

* This site is hosted on [GitHub Pages](https://pages.github.com/) with [Chirpy Theme](https://github.com/cotes2020/jekyll-theme-chirpy/)

## Some Links

* [My portfolio](https://ahampriyanshu.com/)
* [Hail CPP](https://cpp.ahampriyanshu.com/)
* [Algo ds 101](https://dsa.ahampriyanshu.com/)